...
import { createExperience } from '@/app/api/experiences';
...
const CreateExp = ({ profileId, isModalOpen, closeModal, refresh }: UserModalProps) => {
    ...
    const handleSubmit = async (values: any) => {
        ...
        try {
            ...
            console.log("exp->", exp);
            const response = await createExperience(exp);
            console.log("values->", response)
            if (response.statusCode === 201) {
                message.success('exp created successfully!');
                form.resetFields();
                closeModal();
                refresh(1, 5, null);
            }
            ...
            message.error('Failed to create exp.');
        }
        setLoading(false);
    };
