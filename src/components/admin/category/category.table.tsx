...
import CreateDegree from "./category.create";
import EditDegree from "./category.edit";
...
const CategoryTable = (props: IProps) => {
    ...
    const handleActive = async (category: any) => {
        try {
            const response = await deleteCategory(category.id, categoryName);
            if (response.statusCode === 200) {
                message.success(categoryName + ' delete successfully!');
                fetchCategory(1, 5, null);
            }
        } catch (e) {
            console.error('Failed to delete ', categoryName);
